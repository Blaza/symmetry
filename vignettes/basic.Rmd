...
Currently, we have seven test statistics, implemented in R as functions with the
following names:
...
1. `I2U`, given by the formula:
   $$\frac{1}{n\choose4} \sum_{1\leq i<j<a<b\leq n} I\{|X_i - X_j| <
         X_a+X_b\}- I\{|X_i + X_j| < X_a+X_b\}
   $$
1. `I2HU`, given by the formula:
   $$\frac{1}{n^2{n\choose2}} \sum_{1\leq i < j \leq n}\sum_{a,b=1}^n
...
Along them, we have Kolmogorov-Smirnov, Sign and Wilcoxon test statistics as R
functions with the following names:
1. `KS`, which is given by the formula:
  $$
    \sup_t\left|F_n(t)-(1-F_n(-t))\right|
  $$
1. `SGN`, given by the formula:
  $$
    \frac1n \sum_{i=1}^nI\{X_i > 0\} - \frac12
  $$
1. `WCX`, given by the formula:
  $$
